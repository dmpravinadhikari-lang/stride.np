...
- **Real photographs of Shailakshya houses.** Still the single biggest gap.
  §9 requires a real house in the hero, and the house list is also what the
  pre-generation catalogue (§5.2) keys on — without it, most visitors pay a
  live generation instead of hitting cache.

  In the meantime the hero and the style cards run on original vector
  illustrations generated by `scripts/make-reference-art.mjs`. A set of
  third-party architectural renders was offered for this and **not used**: they
  carried other practices' watermarks (ArEiCon Architects, KHD House Design,
  mellani.com.br), several named those practices' private clients in the title
  block, and presenting them as Shailakshya's work would have been both an
  infringement and a misrepresentation to prospects. The drawings avoid all of
  that and, because both hero frames come from one geometry, they register in
  the slider better than photographs would.
